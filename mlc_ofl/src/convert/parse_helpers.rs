...
pub trait ParseExecutorObj {
    fn parse<T>(&self, key: &str) -> ContextResult<T>
    where
        T: Parseable;
    fn parse_default<T>(&self, key: &str, default: T) -> ContextResult<T>
    where
        Option<T>: Parseable;
}
...
pub trait ParseExecutorValue {
    fn parse<T>(&self) -> ContextResult<T>
    where
        T: Parseable;
    #[allow(unused)]
    fn parse_default<T>(&self, default: T) -> ContextResult<T>
    where
        Option<T>: Parseable;
}
